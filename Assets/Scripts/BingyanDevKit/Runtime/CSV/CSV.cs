...
namespace Bingyan
{
    ...
    {
        private static readonly HashSet<string> falseStrs = new HashSet<string>()
        {
            "0", "否", "无", "false"
        };

        private static readonly Dictionary<Type, Func<string, object>> defaultParsers = new Dictionary<Type, Func<string, object>>()
        {
            { typeof(string), s=>s },
            { typeof(int), s=>{
                                    ...
                                    params (Type, Func<string, object>)[] customParsers)
        {
            ...
            lines.ForEachIndex(
                (idx, line) =>
                {
                    ...
                    {
                        Debug.LogWarning($"第 {idx} 行缺少数据，跳过！");
                        return;
                    }
                    ...
                    foreach (var colFieldPair in colFieldMap)
                    {
                        ...
                        {
                            Debug.LogError($"第 {idx} 行第 {colFieldPair.Key + 1} 列解析失败:\n{e.Message}");
                        }
