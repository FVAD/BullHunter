...
namespace Bingyan.Editor
{
    public static class EditorList
    {
        ...
        private static readonly GUIContent icoCreate = new GUIContent(EditorGUIUtility.IconContent("d_Toolbar Plus@2x"));
        private static readonly GUIContent icoRemove = new GUIContent(EditorGUIUtility.IconContent("d_Toolbar Minus@2x"));
        ...
        public static Rect Draw(Rect pos, SerializedProperty property, string label, ref State state)
        {
            ...
            property.arraySize = EditorGUI.DelayedIntField(new Rect(pos.x + pos.width - ARR_SIZE_FIELD_WIDTH, pos.y, ARR_SIZE_FIELD_WIDTH, pos.height), property.arraySize);
            ...
            // 表头
            GUI.Box(new Rect(pos.x, pos.y, pos.width, LIST_PADDING), "", styleRLHeader);
            var insidePos = new Rect(pos.x + LIST_PADDING, pos.y + LIST_PADDING, pos.width - 2 * LIST_PADDING, pos.height);

            // 空列表
            if (property.arraySize == 0)
            {
                GUI.Box(new Rect(pos.x, pos.y + LIST_PADDING, pos.width, LineHeight + LIST_PADDING), "", styleRLBackground);
            ...
            else
            {
                ...
                GUI.Box(new Rect(pos.x, pos.y + LIST_PADDING, pos.width, height - Spacing + LIST_PADDING), "", styleRLBackground);
                ...
                void DrawElement(Rect rect, int index, State state)
                {
                    ...
                    GUI.Box(new Rect(rect.x + 2, rect.y + LineHeight * 0.4f, HANDLE_WIDTH, rect.height), "", "RL DragHandle");
                    EditorGUI.PropertyField(
                        new Rect(rect.x + FIELD_MARGIN + HANDLE_WIDTH, rect.y, rect.width - FIELD_MARGIN - HANDLE_WIDTH, rect.height),
                ...
                if (state.Dragging && Event.current != null)
                    DrawElement(new Rect(rects[0].x,
            ...
            btnAddClicked = GUI.Button(new Rect(footerPos.x + FOOTER_PADDING, footerPos.y, FOOTER_WIDTH / 2 - FOOTER_PADDING * 3 / 2, footerPos.height), icoCreate, styleRLFooterBtn);

            GUI.enabled = property.arraySize > 0;
            btnRemoveClicked = GUI.Button(new Rect(footerPos.x + FOOTER_WIDTH / 2 + FOOTER_PADDING / 2, footerPos.y, FOOTER_WIDTH / 2 - FOOTER_PADDING * 3 / 2, footerPos.height), icoRemove, styleRLFooterBtn);
            GUI.enabled = true;
            ...
            return new Rect(pos.x, footerPos.y + LineHeight + 2 * Spacing, pos.width, LineHeight);
        }
