...
namespace Bingyan.Editor
{
    ...
    {
        private static readonly Dictionary<string, bool> idListFolds = new Dictionary<string, bool>();

        [SettingsProvider]
        public static SettingsProvider Configue()
            => new SettingsProvider("Project/Bingyan/DevKit", SettingsScope.Project)
            {
                label = "DevKit",
                guiHandler = s =>
                {
